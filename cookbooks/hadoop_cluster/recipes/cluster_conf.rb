...
db_server1 = search(:node,"name:hadoop_demo-master1-0")
if (db_server1[0])
puts "db_server1 ="
...
node[:hadoop][:datanode ][:addr] = "#{db_server2[0][:ipaddress]}"
puts "private_ip of datanode ="
puts node[:hadoop][:datanode ][:addr]
end


db_server3 = search(:node,"name:hadoop_demo-work1-0")
if (db_server3 [0])
puts "db_server 3 ="
puts db_server3[0].to_s
node[:hadoop][:nodemanager ][:addr] = "#{db_server3 [0][:ipaddress]}"
